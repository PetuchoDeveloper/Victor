...
use vmath::{euclidian_distance, cosine_similarity};
use uuid::Uuid;
use tokio::fs;
...
impl Collection {
    ...
    pub fn upsert(&mut self, point: Point) -> Result<(), String> {
    ...
    pub fn search(&self, query_vector: Vec<f32>, k: usize) -> Result<Vec<Point>, String> {
        ...
        distances.into_iter().map(|(id, _)| self.hmap.get(&id).unwrap().clone()).collect()

        return Ok(distances);
    }

    pub async fn save_to_disk(&self, filepath: &str) -> Result<(), String> {
